export const assembler = {
	methods: {
		assemble: function(input) {
			...
			const regexInterpreter = /^[\s]?(?:(\w*)[,])?[\s]*(?:(\w*))?[\s]*(?:(-?\w*))?/
			const marieRegex = /^[\s]?(?:(\w*)[,])?[\s]*(?:(\w*))?[\s]*(?:(-?\w*))?/
			const whitespaceRegex = /^\s*(?:\/.*)?$/
			const labelRegex = /^\d.*$/
			const addressRegex = /^\d[0-9a-fA-F]*$/

			const GROUP_LABEL = 1
			const GROUP_OPCODE = 2
			const GROUP_OPERAND = 3
			const GROUP_COMMENT = 4

			const MAX_ADDRESS = 4095 // 2^12 [0000 0000] = 4095
			const MAX_INT = 32767
			const MIN_INT = -32768
			const DEC = -1
			const OCT = -2
			...
			for (var i = 0, l = parsed.length; i < l; ++i) {
				...
				if (directiveBase) {
					...
					if (isNaN(literal)) {
						...
						if (directiveBase == 10) {
							errors.push({
								name: errorMessages[7],
								line: instruction.line
							})
						} else if (directiveBase == 8) {
							errors.push({
								name: errorMessages[8],
								line: instruction.line
							})
						} else if (directiveBase == 16) {
							errors.push({
								name: errorMessages[9],
								line: instruction.line
							})
						}
					}
				...
				var opcode = this.opcodes[instruction.opcode]
